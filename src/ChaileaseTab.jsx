/**
 * Chailease Applications — Branch submits customer details for Chailease merchant
 * financing → Admin/Manager submits to Chailease → Chailease may request follow-up
 * (branch must respond before it can move forward) → Admin marks
 * Approved/Rejected by Chailease. On Approval, a new CCM order is automatically
...
const STEPS=[
  ...
  {step:2,label:"Submitted to Chailease",color:"#7C3AED",bg:"#F5F0FF"},
...
function ApplicationForm({branchMeta,userBranch,isAdmin,srList,editingApp,onSaved,onCancel,onDirtyChange}){
  ...
  const submit=async()=>{
    ...
    setSaving(true);
    const id=editingApp?.id||`chailease_${Date.now()}`;
    const uploadedDocs={};
    ...
    // wrong details before admin ever submitted to Chailease), resubmitting
    // here clears that pending state so it goes back to admin as a normal
    // new application, ready for Submit to Chailease again.
...
function FollowUpResponseBox({app,onSaved}){
  ...
  const respond=async()=>{
    setSaving(true);
    let f=null;
    if(file)f=await readAppFile(file,`chailease_${app.id}_followup`);
  ...
  return<div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:8,padding:10,marginTop:8}}>
    <div style={{fontSize:12,fontWeight:700,color:"#B45309",marginBottom:6}}>Chailease Requested Follow-Up</div>
...
function AdminActions({app,onSaved,onCreateOrder}){
  ...
  const submitToChailease=async()=>{
    setSaving(true);
    await onSaved({...app,step:2,submittedToChaileaseDate:nowDate(),
    ...
    if(app.isDeviceAmendment&&app.deviceAmendment?.orderId){
      ...
      if(order&&order.pendingDeviceAmendment){
        await reconcile([order],[{...order,pendingDeviceAmendment:null,
          history:[...(order.history||[]),{step:order.step,date:nowDate(),time:nowTime(),note:`Device amendment rejected by Chailease: ${rejectedRemark}. Order unchanged.`,skipStepDate:true}]}]);
      }
    }
  ...
  if(app.step===5)return<ActionBox title="Rejected by Chailease">
    <div style={{fontSize:12,color:"#DC2626",fontWeight:600}}>Rejected {fDate(app.rejectedDate)} — {app.rejectedRemark}</div>
  </ActionBox>;

  if(app.step===3&&!app.followUpRespondedDate)return<ActionBox title="Follow-Up Required" desc={`What Chailease needs: ${app.followUpRemark}`}>
  ...
  if(app.step===1)return<ActionBox title="Next: Submit to Chailease" desc="New application ready to be submitted to Chailease for review.">
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {!showAmendment&&<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <PBtn onClick={submitToChailease} disabled={saving} style={{flex:1,justifyContent:"center"}}>{saving?"Saving…":"Submit to Chailease"}</PBtn>
  ...
  // sitting with Chailease awaiting a decision.
  return<ActionBox title="Next: Chailease Decision" desc="Waiting for Chailease to approve, reject, or request more information on this application.">
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {!showFollowUp&&!showApprove&&!showReject?<>
          ...
          <PBtn onClick={()=>setShowApprove(true)} style={{fontSize:12,background:"#15803D",boxShadow:"none"}}>Approved by Chailease</PBtn>
          <GBtn onClick={()=>setShowReject(true)} style={{fontSize:12,color:"#DC2626",borderColor:"#FECACA"}}>Rejected by Chailease</GBtn>
        </>:null}
      </div>
    {showFollowUp&&<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:10}}>
      <L req>Follow-Up Remark (what Chailease needs)</L>
...
function ApplicationDetail({app,branchMeta,isAdmin,canEditDelete,canDelete,onBack,onGoToApp,onSaved,onDelete,onEdit,onCreateOrder,fileUrls}){
  ...
  const copyLink=async()=>{
    const url=`${window.location.origin}${window.location.pathname}?chaileaseId=${app.id}${window.location.hash||"#chaileaseApplications"}`;
...
export default function ChaileaseTab({branchMeta,isAdmin,userBranch,srList=[],email=null}){
  ...
  useEffect(()=>{
    if(loading)return;
    const chaileaseId=new URLSearchParams(window.location.search).get("chaileaseId");
    if(!chaileaseId)return;
    const app=apps.find(a=>a.id===chaileaseId);
    if(app&&(!userBranch||app.branch===userBranch)){
      setView("detail");setSelectedId(chaileaseId);
    }else if(app){
  ...
  const createOrderFromApp=async(app)=>{
    ...
    const order={
      ...
      monthlyInstallment:app.monthlyInstallment,tenure:app.tenure,chaileaseDocuments:app.chaileaseDocuments,
      history:[{step:1,date:nowDate(),time:nowTime(),note:`Submitted — auto-created from approved Chailease Application (${app.id})`}],
    };
  ...
  // Alert — admin hasn't submitted to Chailease within 1 day of the branch's New
  ...
  return<div>
    ...
    {overdueSubmissions.length>0&&<div style={{...card,borderLeft:"3px solid #DC2626",padding:"12px 14px",marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
        ...
        <span style={{fontSize:11,fontWeight:700,color:C.navy,textTransform:"uppercase",letterSpacing:"0.05em"}}>Not Yet Submitted to Chailease</span>
