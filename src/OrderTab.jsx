...
function downloadReport(orders,type,dateFilter,merchantFilter){
  ...
  if(isAgreementReceived){
    ...
    rows=filtered.map((o,i)=>{const h=(o.history||[]).find(h=>h.step===9);const up=calcUpfront(o);const monthly=parseFloat(o.billingData?.monthlyInstallment||o.monthlyInstallment)||0;total1+=up.total;total2+=monthly;return`<tr><td>${i+1}</td><td><b>${o.invoiceNo||"—"}</b></td><td>${shortId(o.id)}</td><td>${o.customerName}</td><td>${o.branch}</td><td>${o.phoneModel}</td><td>${fDate(h?.upfrontPaymentDate)}</td><td>RM ${up.total.toFixed(2)}</td><td>RM ${monthly.toFixed(2)}</td><td>${h?.paymentMethod||"—"}</td><td>${h?.verificationRemark||"—"}</td></tr>`;}).join("");
      ...
      {!branchOk?<div style={{fontSize:12,color:C.textLight,fontStyle:"italic",padding:"2px 0"}}>Waiting for admin to process this step.</div>:<>
        ...
        {nextDef.needsInvoiceNo&&isAdmin&&<>
          ...
          {!isCash&&<div style={{background:C.surface,borderRadius:9,padding:"12px 14px",border:`1px solid ${C.border}`,marginBottom:12}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"8px 0 0",fontWeight:800,color:C.navy}}><span>Upfront Type 1 — Subtotal (Agreement + Stamping + Deposit)</span><span>{fRM(upfront.total)}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"8px 0 4px",borderTop:`1px solid ${C.border}`,marginTop:6,color:C.textMid}}><span>First Monthly Installment</span><span style={{fontWeight:600}}>{fRM(order.billingData?.monthlyInstallment)}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"4px 0 0",fontWeight:800,color:C.navy}}><span>Upfront Type 2 — First Monthly Installment</span><span>{fRM(order.billingData?.monthlyInstallment)}</span></div>
          </div>}
        </>}
  ...
  return<div className="fade-in">
    ...
    {upfront&&!isCash&&<div style={{...card,marginBottom:14}}>
      ...
      <div style={{padding:"10px 16px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"5px 0",borderBottom:`1px solid ${C.border}`,color:C.navy,fontWeight:800}}><span>Upfront Type 1 — Subtotal (Agreement + Stamping + Deposit)</span><span>{fRM(upfront.total)}</span></div>
        {order.billingData?.monthlyInstallment&&<div style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"5px 0",borderBottom:`1px solid ${C.border}`,color:C.textMid}}><span>Upfront Type 2 — First Monthly Instalment</span><span style={{fontWeight:600}}>{fRM(order.billingData.monthlyInstallment)}</span></div>}
