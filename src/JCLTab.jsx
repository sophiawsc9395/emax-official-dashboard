...
function AdminActions({app,onSaved,onCreateOrder}){
  ...
  // Approved/Rejected cycle, same as submitToJCL above.
  const resubmitAfterAmendment=async()=>{
    ...
    await onSaved({...app,step:2,phoneModel:newDeviceName,financePrice:newFinancePrice,submittedToJCLDate:nowDate(),
      deviceAmendment:{...app.deviceAmendment,resubmittedDate:nowDate()},
      history:[...(app.history||[]),{step:2,date:nowDate(),time:nowTime(),note:`Resubmitted to JCL after device amendment: ${previousDeviceName||"—"} → ${newDeviceName}, RM${(previousFinancePrice||0).toFixed(2)} → RM${newFinancePrice.toFixed(2)}`}]});
    setSaving(false);
  };
